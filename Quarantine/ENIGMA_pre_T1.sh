...
# T1
#
cp ../T1.nii.gz ./T1.nii.gz
gunzip T1.nii.gz
bet T1.nii T1_brain.nii -R

###############################################################################
# Register T1 to B-ZERO
#
flirt -interp sinc \
      -sincwidth 7 \
      -noresample \
...
gunzip T1_REG.nii.gz
nii2mnc T1_REG.nii T1_REG.mnc
cp T1_REG.nii T1_REG_F.nii
...
if [ ${METHOD} == "ANTS"]; then 
    ...
    echo "mincANTS 3 -m MI[average_blur.mnc,T1_REG.mnc,1,4] --number-of-affine-iterations 20x20x10 --MI-option 32x16000 --Restrict-Deformation 0x1x0 --affine-gradient-descent-option 0.5x0.95x1.e-4x1.e-4 --use-Histogram-Matching -r Gauss[3,0] -t SyN[0.5] -o Average_OUTPUT.xfm -i 10x10" >> nonlin.sh
    ...
    echo '#! /bin/bash' > nonlin.sh
    echo "module load minc-tools/2011.11" >> nonlin.sh
    echo "nlfit_smr_modelless_trans_mod T1_REG.mnc average_blur.mnc Average_OUTPUT.xfm" >> nonlin.sh
    ...
    echo '#! /bin/bash' > nonlin.sh
    echo "fnirt --ref=T1_REG_F.nii --in=vol0000.nii --iout=FNIRT_NL.nii --fout=field_xyz.nii" >> nonlin.sh
    ...
    echo '#! /bin/bash' > nonlin.sh
    echo "module load minc-tools/2011.11" >> nonlin.sh
    echo "mincANTS 3 -m MI[average_blur.mnc,T1_REG.mnc,1,4] --number-of-affine-iterations 20x20x10 --MI-option 32x16000 --Restrict-Deformation --affine-gradient-descent-option 0.5x0.95x1.e-4x1.e-4 --use-Histogram-Matching -r Gauss[3,0] -t SyN[0.5] -o Average_OUTPUT.xfm -i 10x10" >> nonlin.sh
